...
export default function ItemTypeCreateDialog(props) {
    ...
    async function createCategory() {
        try {
            const response = await axios.post('/api/item-type', { name: category });
            ...
            <Grid container padding={'20px'} justifyContent={'center'}>
                ...
                <Grid container justifyContent={'center'} paddingY={'10px'}>
                    <Typography variant={'body2'}>
                        Eddigi kategóriák:{' '}
                        {props.itemTypes?.map((type, index) => (index !== props?.itemTypes?.length - 1 ? type?.name + ', ' : type?.name))}
                    </Typography>
                </Grid>
                <Grid container justifyContent={'center'} paddingY={'10px'}>
                    <TextField
